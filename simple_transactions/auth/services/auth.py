...
class AuthService:
    ...
    async def authenticate_user(self, username: str, password: str) -> UserModel:
        ...
        if not (
            user
            and self.security_service.verify_password(password, user.hashed_password)
        ):
    ...
    async def verify_token_and_get_user(self, username: str, token: str):
    ...
    async def register_user(self, username: str, password: str, balance: int) -> dict:
        if await self.user_repository.is_username_exists(user_username=username):
            logger.info(
                "Registration attempt failed: user with username {username} already registered."
            )
        ...
        output.pop("hashed_password")
        logger.info(
            f"Registration attempt succeded: new user {output} successfully registered."
        )
        return output
